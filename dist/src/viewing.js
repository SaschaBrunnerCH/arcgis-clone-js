...
(function (factory) {
     ...
     */
    function getTopLevelItemIds(items) {
        // Find the top-level nodes. Start with all nodes, then remove those that other nodes depend on
        var topLevelItemCandidateIds = Object.keys(items);
    ...
    function getItemHierarchy(items) {
        var hierarchy = [];
        // Find the top-level nodes. Start with all nodes, then remove those that other nodes depend on
        var topLevelItemIds = getTopLevelItemIds(items);
     ...
     */
    function createSolutionStorymap(title, solution, orgSession, folderId, access) {
    ...
    function createSolutionStorymapItem(title, solution, folderId) {
        ...
        topLevelItemIds.forEach(function (topLevelItemId) {
            var solutionItem = solution[topLevelItemId];
     ...
     * @protected
     */
    function publishSolutionStorymapItem(solutionStorymap, orgSession, folderId, access) {
        if (folderId === void 0) { folderId = null; }
        if (access === void 0) { access = "private"; }
        return new Promise(function (resolve, reject) {
            mCommon.createItemWithData(solutionStorymap.item, solutionStorymap.data, orgSession, folderId, access)
                ...
                var solutionStorymapUrl = orgSession.orgUrl + "/apps/MapSeries/index.html?appid=" + solutionStorymapId;
